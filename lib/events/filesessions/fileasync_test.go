...
func TestUploadOK(t *testing.T) {
	...
	inEvents := events.GenerateSession(events.SessionParams{PrintEvents: 1024})
...
// for semaphore that limits parallel
func TestUploadParallel(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		sessionEvents := events.GenerateSession(events.SessionParams{PrintEvents: 1024})
