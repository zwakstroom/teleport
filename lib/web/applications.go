...
func (h *Handler) createAppSession(w http.ResponseWriter, r *http.Request, p httprouter.Params, ctx *SessionContext) (interface{}, error) {
	...
	// TODO(russjones): What if the session allowed in local cluster is shorter
	// than that allowed in the remote cluster or what to do f the session in
	// the local cluster is longer than that allowed in the remote cluster.
