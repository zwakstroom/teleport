...
func (s *Suite) TestHandleConnection(c *check.C) {
	...
	// Wait for the server to reply and make sure the response contains the
	// UUID that the server replied with earlier.
	select {
	case <-s.closeContext.Done():
	case <-time.After(5 * time.Second):
		c.Fatal("Timed out waiting for target server to respond.")
	}
