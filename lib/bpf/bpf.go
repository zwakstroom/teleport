...
func (s *Service) emitCommandEvent(eventBytes []byte) {
	...
	case eventRet:
		...
		ctx.Emitter.EmitAuditEvent(ctx.Context, sessionCommandEvent)
...
func (s *Service) emitDiskEvent(eventBytes []byte) {
	...
	ctx.Emitter.EmitAuditEvent(ctx.Context, sessionDiskEvent)
}
...
func (s *Service) emit4NetworkEvent(eventBytes []byte) {
	...
	ctx.Emitter.EmitAuditEvent(ctx.Context, sessionNetworkEvent)
}
