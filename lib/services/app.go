...
type App interface {
	...
	// GetHostUUID gets the UUID of the application service.
	GetHostUUID() string
	// SetHostUUID sets the UUID of the application service.
	SetHostUUID(string)
...
// GetInternalAddr gets the internal address of the application.
func (s *AppV3) GetInternalAddr() string {
	return s.Spec.InternalAddr
}

// SetInternalAddr sets the internal address of the application.
func (s *AppV3) SetInternalAddr(internalAddr string) {
...
func (s *AppV3) String() string {
	return fmt.Sprintf("App(name=%v, protocol=%v, internal=%v, public=%v, labels=%v)",
		s.Metadata.Name, s.Spec.Protocol, s.Spec.InternalAddr, s.Spec.PublicAddr, s.GetAllLabels())
}
