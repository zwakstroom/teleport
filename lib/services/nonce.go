...
// Nonce represents a nonce that can be exchanged for a web session. Used
// to transfer authentication information from the Web UI to an application.
type Nonce interface {
	// Resource provides common resource headers.
	Resource

	// GetUsername gets the identity of the user.
	GetUsername() string
	// SetUsername sets the identity of the user.
	SetUsername(string)
...
// NewNonce returns a new services.NonceV3.
...
// GetParentHash gets the hash of the parent session.
func (s *NonceV3) GetParentHash() string {
	return s.Spec.ParentHash
}

// SetParentHash sets the hash of the parent session.
func (s *NonceV3) SetParentHash(parentHash string) {
	s.Spec.ParentHash = parentHash
}
