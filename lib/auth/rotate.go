...
func (r *RotateRequest) Types() []services.CertAuthType {
	switch r.Type {
	case "":
		return []services.CertAuthType{services.HostCA, services.UserCA, services.JWT}
	...
	case services.JWT:
		return []services.CertAuthType{services.JWT}
	}
	return nil
}
...
func (r *RotateRequest) CheckAndSetDefaults(clock clockwork.Clock) error {
	...
	switch r.Type {
	case "", services.HostCA, services.UserCA, services.JWT:
	default:
...
func (a *AuthServer) autoRotateCertAuthorities() error {
	...
	for _, caType := range []services.CertAuthType{services.HostCA, services.UserCA, services.JWT} {
...
func startNewRotation(req rotationReq, ca services.CertAuthority) error {
	...
	if len(req.privateKey) != 0 {
		...
		jwtPublicKey, jwtPrivateKey, err = jwt.MarshalKeypair(rsaKey.(*rsa.PrivateKey))
		if err != nil {
			return trace.Wrap(err)
		}
	} else {
