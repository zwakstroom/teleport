...
func (s *ResetPasswordTokenTest) TearDownSuite(c *check.C) {
}
...
func (s *ResetPasswordTokenTest) TearDownTest(c *check.C) {
}
...
func (s *ResetPasswordTokenTest) TestFormatAccountName(c *check.C) {
	var tests = []struct {
		...
		// Proxies with no public address.
		{
			inDebugAuth: &debugAuth{
				proxies: []services.Server{
					&services.ServerV2{
	...
	for _, tt := range tests {
		...
		c.Assert(accountName, check.Equals, tt.outAccountName)
	}
}
